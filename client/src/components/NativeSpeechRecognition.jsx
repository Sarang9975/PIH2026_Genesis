...
const NativeSpeechRecognition = ({
    ...
    if (SpeechRecognition) {
      ...
      try {
        ...
        recognitionRef.current.onend = () => {
          ...
          if (isAutoStartedRef.current && recognitionRef.current) {
            console.log('🔄 Attempting auto-restart...');
            setDebugInfo('Auto-restarting speech recognition...');
            autoStartTimeoutRef.current = setTimeout(() => {
              if (recognitionRef.current && isAutoStartedRef.current) {
                try {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Note: onTextChange is already called above for interim/final results
          // Do NOT call it again here to avoid flooding translation APIs
        };
  ...
  const stopListening = useCallback(() => {
    try {
      if (recognitionRef.current && isListening) {
        recognitionRef.current.stop();
        isAutoStartedRef.current = false;
  ...
  // Handle mute and active state changes
  useEffect(() => {
    if (!isSupported) return;

    // We should listen if the component is active AND not muted
    const shouldListen = isActive && !isMuted;

    // If we should be listening but aren't, force a start regardless of auto-start state
    if (shouldListen && !isListening) {
      console.log("🎤 Starting transcription because isActive=" + isActive + " and isMuted=" + isMuted);
      startListening();
    } else if (!shouldListen && isListening) {
      console.log("⏸️ Pausing transcription because isActive=" + isActive + ", isMuted=" + isMuted);
      stopListening();
    }
  }, [isMuted, isActive, isListening, isSupported, startListening, stopListening]);

  // Auto-start speech recognition when component mounts
  useEffect(() => {
    if (isSupported && !isAutoStartedRef.current && isActive && !isMuted) {
  ...
  }, [isSupported, isActive, isMuted, startListening]);
